...
Open with one brief Traditional Chinese greeting that fits the travel context, then lead immediately with the recommendation and its decision-relevant limitation. Do not open with the workflow, browser actions, search keywords, sort controls, page coverage, repository or skill-installation details, or a diary of how the evidence was collected. Avoid phrases such as `我先搜尋`, `我再切成訪問月順`, `我查看了幾頁`, or `我使用了以下方法`. Put execution status, browser limitations, search method, retrieval date, and general research caveats after the recommendation as a short note, unless a limitation changes a particular recommendation, in which case mention only that caveat alongside the affected candidate. If the user asks how the research was done, provide that audit separately after the recommendation.
...
User-facing output must follow [user-facing-style.md](../../sites/tabelog/references/user-facing-style.md) as a hard output gate. Write one coherent first-person Traditional Chinese article in the voice of a local tourism ambassador. Use a Traditional Chinese or Taiwan-familiar transliteration as the primary shop and product name, and use `中文名稱（原文名稱）` only when the original helps identification. A raw Japanese name must never be the primary display label. Let the Tabelog link carry the formal Japanese name. Embed Tabelog, official-site, and SNS links naturally inside the relevant sentence. Do not use Markdown headings, bullets, numbered lists, tables, bold, italics, or a separate link appendix. Use Chinese commas and full stops for prose. Do not use semicolons or em dashes in the user-facing answer.

Before returning, run the final language QA: every shop and product has `name_zh` or a Taiwan-familiar transliteration, no raw Japanese name appears outside parentheses, source links, or required search terms, the answer is one connected article in first person, the voice is that of a tourism ambassador, and the answer contains no semicolon. If any candidate fails, normalize it or rewrite the article before returning. Do not use the source language as a reason to skip this gate.

Integrate the default score-first Top N into the same article without numbered labels or separate recommendation cards. Use natural transitions to move between candidates. If a specialist or more practical candidate would replace a score-first candidate, explain the trade-off in the article rather than creating a second ranking. Keep near-misses brief and give each one primary exclusion reason.
...
- [User-facing style](../../sites/tabelog/references/user-facing-style.md) — first-person tourism-ambassador voice, article format, and punctuation gate.
