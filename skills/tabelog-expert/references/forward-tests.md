...
| Greeting opening | User asks for a conversational travel recommendation | The answer begins with one brief Traditional Chinese greeting that fits the travel context, then moves directly into the recommendation without method narration or empty pleasantries. |
| Tourism ambassador article | User asks for recommendations in a conversational travel context | The answer uses first-person Traditional Chinese in one connected tourism-ambassador article, integrates all requested candidates with natural transitions, and does not use bullets, numbered labels, tables, or separate recommendation cards. |
...
- The final response is one connected first-person Traditional Chinese tourism-ambassador article with Taiwan-familiar store names and naturally embedded Markdown links. It does not use Markdown headings, bullets, numbered lists, tables, bold, italics, semicolons, em dashes, or a separate link appendix.
